---
name: 02-headlines-hooks
description: Write headlines and opening hooks that earn the click and the first scroll, grounded in organic CTR data and reading-pattern research. Apply after ../00-blog-writing-guardrails/SKILL.md, before drafting or editing any blog post's title, meta title, or opening paragraphs.
version: 1.0.0
...
Apply `../00-blog-writing-guardrails/SKILL.md` first; its hard gates (originality, accuracy, E-E-A-T, accessibility, plain language, structured data, pre-publish QA) apply to every headline and hook produced under this skill.

## 1. Title-length gate
- Keep the page title / title tag between roughly 40 and 60 characters; titles in this range get a measured 8.9% higher average CTR than titles outside it. [BACKLINKO-CTR-01]
- If a title must run longer to stay accurate, front-load the words that carry the meaning inside the first ~60 characters, since search engines and readers may truncate anything after. [BACKLINKO-CTR-01]

## 2. Sentiment and honesty gate
- Prefer a positive or neutral framing over a negative one when the underlying content supports it; positive-sentiment titles measured a 4.1% higher absolute CTR than negative ones. [BACKLINKO-CTR-01]
- Do not force positive framing onto content that is genuinely a warning, failure, or critique — a mismatched headline breaks the accuracy and originality gates it must still pass. [GOOGLE-EEAT-01]
- Never publish a headline whose implied claim ("proven," "best," "#1," a specific result) is not something the author can substantiate on request; this is the same advertising-substantiation gate applied to the first line the reader sees. [FTC-ADSUB-01]
...
- Where volume and tooling allow (a title A/B test, a headline-testing tool, or a controlled send to a subset of an owned audience), test more than one headline variant against real readers before finalizing, rather than shipping on editorial instinct alone; large-scale controlled headline testing consistently outperforms guessing at what will get clicks. [UPWORTHY-ARCHIVE-01]
- Where no testing infrastructure exists, apply the length, sentiment, and question gates above as the default heuristic instead of guessing freely. [BACKLINKO-CTR-01]
